...
import { LineChart, Line, XAxis, YAxis, Tooltip, ResponsiveContainer, CartesianGrid, ReferenceLine, BarChart, Bar as RBar } from "recharts";

const P = PRODUCTS.prahari;
const PD_TREND = [
...
const DRIVERS = [
  ...
  { f: "Delayed EMI (2x in 6 mo)", w: 0.18 },
  { f: "Power consumption down 40%", w: 0.14 },
...
export default function Prahari() {
  ...
  return (
    <div style={{ "--pa": P.accent }}>
      <ProductHeader pkey="prahari" right={<span className="phead-pill">Predict before stress</span>} />
      <div className="wrap">
        <div className="grid g4" style={{ marginBottom: 20 }}>
          <div className="stat" style={{ borderTop: `3px solid ${P.accent}` }}><div className="lbl">Portfolio monitored</div><div className="val" style={{ color: P.accent }}>₹4.8k Cr</div><div className="delta">MSME loan book</div></div>
          <div className="stat" style={{ borderTop: "3px solid #D64545" }}><div className="lbl">Accounts at Red</div><div className="val" style={{ color: "#D64545" }}>142</div><div className="delta down">flagged 12-mo early</div></div>
          <div className="stat" style={{ borderTop: `3px solid ${P.accent}` }}><div className="lbl">Model AUC-ROC</div><div className="val" style={{ color: P.accent }}>0.89</div><div className="delta up">not raw accuracy</div></div>
          <div className="stat" style={{ borderTop: `3px solid ${P.accent}` }}><div className="lbl">Lead time gain</div><div className="val" style={{ color: P.accent }}>+9 mo</div><div className="delta up">vs current 3-mo model</div></div>
        </div>

        <div className="card" style={{ marginBottom: 18, background: P.soft, borderColor: P.accent2 }}>
          <div style={{ fontSize: 12.5 }}><b>Why we do not report raw accuracy:</b> default data is highly imbalanced.
        ...
        <div className="grid" style={{ gridTemplateColumns: "1fr 400px", alignItems: "start" }}>
          <div className="card">
            ...
            <table>
              ...
              <tbody>
                {PORTFOLIO.map((p) => (
                  <tr key={p.id} onClick={() => setSel(p)} style={{ cursor: "pointer", background: sel.id === p.id ? P.soft : "" }}>
            ...
            <div style={{ marginTop: 16 }}>
              <div className="sectitle acc">Model Performance (synthetic-data prototype)</div>
              <div className="grid g3">
                {METRICS.map(([k, v]) => (
                  <div key={k} className="softcard" style={{ padding: "10px 12px" }}>
                    <div className="tsub">{k}</div><div style={{ fontSize: 18, fontWeight: 800, color: P.accent }}>{v}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="card">
              ...
              <ResponsiveContainer width="100%" height={180}>
                <LineChart data={PD_TREND}>
                  ...
                  <Line type="monotone" dataKey="pd" stroke={P.accent} strokeWidth={2.5} dot={{ r: 3 }} />
                </LineChart>
              </ResponsiveContainer>
            </div>
            <div className="card">
              ...
              <ResponsiveContainer width="100%" height={185}>
                <BarChart data={DRIVERS} layout="vertical" margin={{ left: 10 }}>
                  ...
                  <Tooltip /><RBar dataKey="w" radius={[0, 5, 5, 0]} fill={P.accent2} />
                </BarChart>
              </ResponsiveContainer>
            </div>
            <div className="card" style={{ background: P.soft }}>
